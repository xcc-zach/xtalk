...
class _InferenceSnapshot:
    """Cache one raw model action with its exact inference inputs."""

    dialogue: tuple[tuple[str, str], ...]
    state: str
    speech_pause_seen: bool
    action: str
...
class XTurnix(TurnDetector):
    ...
    inference by themselves. Each new cumulative user ASR boundary and each
    assistant interruption trigger one constrained XTurnix action prediction.
    """
    ...
    def __init__(
        ...
        self._last_model_inference: _InferenceSnapshot | None = None
    ...
    async def async_detect(
        ...
        assistant_interrupted : bool, optional
            Whether assistant speech has just been interrupted before normal
            playback completion. This re-evaluates the latest user revision in
            listening state.

        Returns
        -------
        TurnDetectionResult
            Turn action selected for the newest user-text revision. Assistant
            text-only calls return ``DO_NOTHING`` with ``IDLE``; interruption
            calls re-evaluate the latest user revision.
        """
        del audio
        async with self._state_lock:
            self._sync_listening_state_locked()

            if speech_start and self._mark_user_speech_start_locked():
            ...
            should_infer = text is not None or assistant_interrupted
            if text is not None:
                ...
                if changed:
                    self._user_revision += 1
            elif speech_pause and self._mark_user_pause_locked():
                self._user_revision += 1
    ...
    def _begin_stream_locked(stream: _StreamState) -> None:
        ...
        stream.epoch += 1
        stream.text = ""
        stream.speech_pause_seen = False
    ...
    def _mark_user_speech_start_locked(self) -> bool:
        """Close the action gate until the current VAD segment pauses."""
        self._ensure_user_stream_locked()
        if not self._user_stream.speech_pause_seen:
            return False
        self._user_stream.speech_pause_seen = False
        return True
    ...
    def _mark_user_pause_locked(self) -> bool:
        ...
        self._ensure_user_stream_locked()
        pause_state_changed = not self._user_stream.speech_pause_seen
        self._user_stream.speech_pause_seen = True
    ...
    async def _infer_latest_user_revision(self) -> TurnDetectionResult:
        """Coalesce partials while preserving stop actions during speaking."""
        async with self._inference_lock:
            while True:
                async with self._state_lock:
                    ...
                    speech_pause_seen = self._user_stream.speech_pause_seen
                ...
                dialogue_key = tuple(
                    (message["role"], message["content"]) for message in dialogue
                )
                cached = self._last_model_inference
                reuse_previous = (
                    speech_pause_seen
                    and cached is not None
                    and not cached.speech_pause_seen
                    and cached.dialogue == dialogue_key
                ...
                if reuse_previous:
                    ...
                    self._last_model_inference = _InferenceSnapshot(
                        dialogue=dialogue_key,
                        state=state,
                        speech_pause_seen=speech_pause_seen,
                        action=action,
                    )

                accepted_stale_stop = False
                async with self._state_lock:
                    self._sync_listening_state_locked()
                    latest_key = (
                        self._user_revision,
                    ...
                    accepted_stale_stop = (
                        latest_key != inference_key
                        and action == _STOP_ACTION
                        and not listening
                        and latest_key[1] == inference_key[1]
                    )
                    if latest_key != inference_key and not accepted_stale_stop:
                        result = None
                    else:
                        self._last_inferred_key = latest_key
                        result = self._map_action_locked(
                            action,
                            listening,
                            speech_pause_seen,
                        )
                ...
                decision = (
                    ...
                    else TurnDetectionAction.DO_NOTHING
                )
                state_name = "listening" if listening else "speaking"
                logger.debug(
                    "XTurnix model inference completed; last user input: %r; "
                    "model output: %r; turn detector decision: %s; state: %s; "
                    "reused previous output: %s; accepted stale stop: %s",
                    last_user_text,
                    action,
                    decision.name,
                    state_name,
    ...
    async def _infer_action(
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            request_dialogue = dialogue
            if request_dialogue and request_dialogue[-1]["role"] == "assistant":
                request_dialogue = request_dialogue[:-1]
            messages = [
                {
                    "role": "system",
    ...
    def _map_action_locked(
        ...
        if listening:
            # Guard against false <|start|> predictions while the user is still speaking.
            if action == _START_ACTION and speech_pause_seen:
