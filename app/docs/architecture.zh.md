...
未启用语音唤醒时，关闭主窗口会执行完整退出流程。启用后，关闭请求会先要求
WebView 关闭当前 Session 并释放会话麦克风；WebView 随后恢复原生唤醒监听，并在
检测进程明确进入 `listening` 状态后才要求 Tauri 将窗口隐藏到系统托盘。沉睡工具
会在最后一句告别语播放完成后复用同一套确认流程。Session 关闭或唤醒进程启动失败
时，窗口会保持可见并显示错误，不会进入只完成一半的后台状态。
如果唤醒进程在进入后台后意外退出，原生 supervisor 会重新显示主窗口，避免 App
停留在无法唤醒也无法操作的沉睡状态。托盘中的“退出”会停止唤醒词进程，再调用
受认证的 Python sidecar shutdown endpoint 并在有限时间内等待；若优雅关闭失败，
只终止 App 自己启动的子进程。随后按启动顺序的逆序停止 managed 模型进程。
