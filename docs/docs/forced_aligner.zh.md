...
每个被接受的文本增量都会携带该增量发送前已经准备完成的、经过语速处理的 PCM 时长。L1 只把本次新增文本转换为不可变时间单元，其起点不得早于该音频准备水位、已经发送或播放的音频位置以及上一单元的结束位置。仅接受新文本不会推进`ResponseUpdate`；只有前端确认的播放位置越过单元的固定结束时间和安全延迟后，该单元才会显示。因此，后续文本不会把已经播放的时间重新分配到扩大的文本缓冲区上。

StreamingTextTTS L1 固定使用一组更保守的估算值：中文每字 280 ms、英文每词 380 ms、每个标点 160 ms，以及 300 ms 播放安全延迟。
